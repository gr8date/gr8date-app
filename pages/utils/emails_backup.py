...
def send_reminder_email(user_email, username, complete_profile_url):
    """Send reminder email to complete profile"""
    try:
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    <p>We noticed you haven't completed your GR8DATE profile yet. You're just a few steps away from connecting with amazing people!</p>
                    
                    <p>Complete your profile to:</p>
                    <ul>
                        <li>✅ Get approved faster</li>
                        <li>✅ Appear in search results</li>
                        <li>✅ Receive more matches</li>
                        <li>✅ Unlock all features</li>
                    </ul>
                    
                    <p style="text-align: center;">
                        <a href="{complete_profile_url}" class="button">Complete My Profile</a>
                    </p>
                    
                    <p>If you need any help, just reply to this email!</p>
                </div>
                <div class="footer">
                    <p>GR8DATE - Find Your Perfect Match</p>
                    <p><a href="https://gr8date.com.au">gr8date.com.au</a></p>
                </div>
            </div>
        </body>
        </html>
        ...
        We noticed you haven't completed your GR8DATE profile yet. 
        ...
        Complete your profile here: {complete_profile_url}
        
        If you need any help, just reply to this email!
        
        GR8DATE - Find Your Perfect Match
        gr8date.com.au
        """
        ...
        print(f"DEBUG: Error sending reminder email: {e}")
        return False
...
try:
    from pages.models import EmailReminderLog
    REMINDER_LOG_AVAILABLE = True
except ImportError:
    REMINDER_LOG_AVAILABLE = False

def send_verification_reminder(user, request=None):
    """Send email verification reminder using Brevo"""
    try:
        subject = "Please verify your email address"
        
        # Create verification URL
        verification_url = f"{getattr(settings, 'SITE_URL', 'http://localhost:8000')}/verify-email/{user.profile.email_verification_token}/"
        ...
        # Use your existing Brevo function to send the email
        ...
        # Log the reminder if available
        if REMINDER_LOG_AVAILABLE and success:
            EmailReminderLog.objects.create(
                user=user,
                reminder_type='email_verification',
                was_successful=True
            )
        return success
        
    except Exception as e:
        # Log the failure if available
        if REMINDER_LOG_AVAILABLE:
            EmailReminderLog.objects.create(
                user=user,
                reminder_type='email_verification',
...
def send_profile_completion_reminder(user, request=None):
    """Send profile completion reminder using Brevo"""
    try:
        subject = "Complete Your Dating Profile"
        
        # Create profile URL
        profile_url = f"{getattr(settings, 'SITE_URL', 'http://localhost:8000')}/profile/edit/"
        ...
        <body>
            ...
            <p>Hello {user.username},</p>
            <p>Your dating profile is almost ready! Complete your profile to start matching with other users.</p>
            <p><a href="{profile_url}">Complete Your Profile</a></p>
            <p>Add photos, tell us about yourself, and set your preferences to get better matches.</p>
        </body>
        </html>
        """
        ...
        print(f"Failed to send profile completion reminder: {e}")
        return False

# Keep your existing send_password_reset_email function if it exists
# If not, you can add it here or it might already be in your file
